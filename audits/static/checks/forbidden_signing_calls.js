...
// Scans all JS/TS source for forbidden signing/private-key patterns.
...
const CHECK_NAME = "static.forbidden_signing_calls";
...
export async function run(ctx) {
  ...
  if (hits.length > 0) {
    addCheck(ctx, {
      ...
      details: `${hits.length} forbidden signing call(s) found: ${hits.slice(0, 3).map(h => `${h.pattern} @ ${h.file}:${h.line}`).join("; ")}`,
    ...
    addCheck(ctx, {
      ...
      details: `No forbidden signing patterns found (scanned ${FORBIDDEN_SIGNING_PATTERNS.length} patterns)`,
      durationMs: Date.now() - start,
    });
  }
