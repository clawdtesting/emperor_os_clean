    ...
    <div className="flex flex-wrap gap-2">
      {tabs.map(t => (
        <button
          key={t}
          onClick={() => setTab(t)}
          className={`px-3 py-1.5 rounded text-xs border ${tab === t ? 'bg-blue-600 text-white border-blue-500' : 'border-slate-700 text-slate-300 hover:bg-slate-800'}`}
        >
          {t}
        </button>
      ))}
...
function OperationsDeckView({ jobsDesc, assigned, completed, disputed, selected, setSelected, tab, setTab, ...rest }) {
  return (
    <div className="p-4 space-y-4">
          ...
          <div className="space-y-2">
            {leftTabs.map(name => (
              <button
                key={name}
                onClick={() => setTab(name)}
                className={`w-full text-left px-3 py-2 rounded-lg text-xs uppercase tracking-wide border transition ${tab === name ? 'bg-violet-600/80 text-white border-violet-400' : 'bg-slate-900/60 text-slate-300 border-slate-700 hover:border-violet-700'}`}
              >
                {name}
              </button>
            ))}
